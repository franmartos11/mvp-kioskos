...
-- Allow any authenticated user to create a new kiosk (needed for registration)
...
drop policy if exists "Owners can update their kiosks" on public.kiosks;
create policy "Owners can update their kiosks"
on public.kiosks for update
using ( auth.uid() = owner_id );

-- Allow owners to delete their kiosks
drop policy if exists "Owners can delete their kiosks" on public.kiosks;
...
drop policy if exists "Manage kiosk members" on public.kiosk_members;
